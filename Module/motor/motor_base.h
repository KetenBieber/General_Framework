 ...
 * @brief
 * 电机封装的优化，目前打算将某些数据存进CCMRAM，提高读写能力，以及将部分函数进行inline，减少函数调用的开销
 ...
 *
 * @attention : 注意inline
 * 关键字不会自动继承，所以需要自行在派生类中添加inline声明
...
class Motor {
public:
  Motor(uint8_t id, const CAN_Rx_Instance_t &can_rx_instance,
...
public:
  ...
  virtual void
  CanMsg_Process(CAN_Tx_Instance_t &can_txmsg) = 0; // can帧接收数据解包
