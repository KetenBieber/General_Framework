 ...
 * 哪些按键就有使用者自己写
 * 具体需要在xbox.h中的XboxControllerData_t->pub_Xbox_Data xbox_data;跳转到pub_Xbox_Data里
...
uint8_t Xbox_Init(Uart_Instance_t *xbox_uart_instance)
{
    ...
    XBOX_Instance->xbox_msgs = (XboxControllerData_t*)pvPortMalloc(sizeof(XboxControllerData_t));
...
void Xbox_Get_Data(uint8_t *xbox_datas, XBOX_Instance_t *Instance)
{
    ...
    Instance->xbox_msgs->joyLHori = ((uint16_t)xbox_datas[16] << 8) | xbox_datas[17];
    Instance->xbox_msgs->joyLVert = ((uint16_t)xbox_datas[18] << 8) | xbox_datas[19];
    Instance->xbox_msgs->joyRHori = ((uint16_t)xbox_datas[20] << 8) | xbox_datas[21];
    Instance->xbox_msgs->joyRVert = ((uint16_t)xbox_datas[22] << 8) | xbox_datas[23];
    Instance->xbox_msgs->trigLT = ((uint16_t)xbox_datas[24] << 8) | xbox_datas[25];
    Instance->xbox_msgs->trigRT = ((uint16_t)xbox_datas[26] << 8) | xbox_datas[27];
...
uint8_t Xbox_Uart_Rx_Callback(Uart_Instance_t *uart_instance, uint16_t data_len)
{
    ...
    // memcpy(aaa, temp_uart_instance->uart_package.rx_buffer, sizeof(temp_uart_instance->uart_package.rx_buffer));
